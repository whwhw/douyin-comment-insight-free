...
将视频口播转成可阅读、带时间定位的文案，并继续完成结构拆解、分段点评和二创分析。已经通过检查的同服务转写可以复用，避免重复处理。

底层语音识别由用户自己配置的火山引擎服务提供。本机仍需 FFmpeg 和 FFprobe 处理音频。免费版提供接入能力，不包含服务密钥或转写额度。

实际速度受音频长度、网络和服务状态影响，不承诺秒出或固定倍速。当前尚未完成真实火山服务的速度验收，相关宣传数字需实测后再确定。
