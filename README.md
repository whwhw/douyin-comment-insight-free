...
先体验账号体检：安装技能与 Python 依赖，配置 TikHub 即可开始。音频工具和模型只在提取口播时需要，缺少它们不会阻止账号体检。分析由 Codex 按 SKILL.md 执行。
...
首次默认观察 3 条作品、每条最多请求 30 条评论，先交付内容方向、样本内作品表现和最多 3 个有原文证据的讨论点。数据不足时如实说明，不凑数，也不把小样本结论当成整个账号的情况。

体检完成后，本地环境就绪时再提取 1 条代表作品的口播。需要更多样本时，可以明确要求扩大采集范围（最多 30 条作品、每条最多 100 条评论）。

## 第一次怎么用

解压后用 Codex 打开文件夹，复制这句话：

```text
请阅读 README.md，帮我安装这个技能和 Python 依赖，打开安装目录的 .env，指导我填写 TikHub 配置，不要显示真实密钥。先完成账号体检所需的配置。
```

安装完成后发送：

```text
使用 $douyin-comment-insight-free，给抖音号 xxx 做一次账号体检。先给我简评和文件，再在本地环境就绪时提取一条代表作品的口播。
```

将 xxx 换成真实抖音号。你会先拿到对话中的简评和 account-brief.md；口播提取是后续增强步骤。缺少模型时会说明缺项，你可以稍后继续配置。
...
- `account-brief.md`：可阅读、保存的账号体检简报，先于转写交付。
...
已完成小规模账号采集、本地中文转写、TXT/SRT 导出和转写复用测试。轻量体检另有离线端到端检查，验证缺少本地模型时仍可保存简报。macOS 安装与升级已经验证；Windows 实机兼容性仍待验收。
