...
def inspect(account):
    ...
    return {'accountId':account,'edition':EDITION,'analysisInput':str(matches[0]) if matches else None,'detailPage':str(WORKSPACE/'site'/f'{account}.html') if (WORKSPACE/'site'/f'{account}.html').exists() else None}
def prepare(account,resume=None,works=30,comments=100,offline=None):
    ...
    try:
        ...
        print(f'采集未完成，旧报告未覆盖。续跑：python scripts/workflow.py prepare --account {account} --resume "{directory}"',file=sys.stderr);raise
    atomic_json(manifest,state)
    return {'accountId':account,'runDir':str(directory),'analysisInput':str(directory/'analysis-input.json'),'findings':str(directory/'findings.json'),'workFindings':str(directory/'work-findings.json'),'finalAnalysis':str(directory/'analysis.json')}
