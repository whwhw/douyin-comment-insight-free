...
def render(data):
    rows=[data.get('displayName',data['accountId'])+' · 基础账号简评',data['summary'],'','作品表现（仅为本次样本内排名）：']
    for w in data.get('works',[])[:5]:rows.append(f"- {w.get('title','')}：赞 {w.get('diggCount',0)}，藏 {w.get('collectCount',0)}，分享 {w.get('shareCount',0)}")
...
if __name__=='__main__':
    ...
    atomic_json(args.output,data);print(render(data))
